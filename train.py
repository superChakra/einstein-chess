...
class SelfPlay:
    def __init__(self, model, c_puct=1.0, num_simulations=1600, num_games=100, num_threads=4):
        """
        初始化自我博弈
        :param model: 策略-价值模型
        :param c_puct: 探索系数
        ...
        """
        self.model = model
        # 初始化MCTS，并传递encode_action方法
        self.mcts = MCTS(model, self.encode_action, c_puct=c_puct, num_simulations=num_simulations, num_threads=num_threads)
    ...
    def get_mcts_action_probs(self, root, current_player, game_state):
        ...
        for action, child in root.children.items():
            try:
                ...
                if 0 <= action_index < len(action_probs):
...
def prepare_training_data(training_data):
    ...
    print(f"States shape after stacking: {states.shape}")  # 应输出 torch.Size([N, 4, 5, 5])

    # 将 policies 转换为 numpy.ndarray 后再转换为 torch.Tensor
    policies = torch.tensor(np.array(policies), dtype=torch.float32)  # 形状 [N, 18]
    # print(f"Policies shape: {policies.shape}")  # 应输出 torch.Size([N, 18])

    rewards = torch.tensor(rewards, dtype=torch.float32).unsqueeze(1)  # 形状 [N, 1]
    # print(f"Rewards shape: {rewards.shape}")  # 应输出 torch.Size([N, 1])
...
if __name__ == '__main__':
    ...
    # 执行自我博弈，生成训练数据
    self_play_agent = SelfPlay(model, c_puct=1.0, num_simulations=1600, num_games=10, num_threads=4)
